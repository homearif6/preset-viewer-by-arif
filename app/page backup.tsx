...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const slidersConfig = [ 
      { id: 'exposure', label: 'Exposure', value: exposure, setter: setExposure, min: -100, max: 100, step: 5 }, 
      { id: 'whiteBalance', label: 'White Balance', value: whiteBalance, setter: setWhiteBalance, min: -100, max: 100, step: 5 }, 
      { id: 'highlights', label: 'Highlights', value: highlights, setter: setHighlights, min: -100, max: 100, step: 5 }, 
      { id: 'shadows', label: 'Shadows', value: shadows, setter: setShadows, min: -100, max: 100, step: 5 }, 
      { id: 'grain', label: 'Grain', value: grain, setter: setGrain, min: 0, max: 100, step: 5 }, 
    ]; 
                    ...
                    {previewImage && ( 
                        <div className="flex flex-col md:flex-row gap-6 p-4 sm:p-6 flex-grow min-h-0"> 
                            ...
                            <div className="flex-grow order-first md:order-last relative flex items-center justify-center min-h-[300px] md:min-h-0"> 
                                <div ref={sliderContainerRef} style={{ aspectRatio }} className="relative w-full max-w-full max-h-full rounded-lg overflow-hidden select-none touch-pan-y shadow-inner bg-gray-200"> 
                                    <canvas ref={afterCanvasRef} className="absolute inset-0 w-full h-full object-contain" /> 
                                    <div className="absolute inset-0 w-full h-full" style={{ clipPath: `inset(0 ${100 - sliderPosition}% 0 0)` }}> 
                                        <canvas ref={beforeCanvasRef} className="absolute inset-0 w-full h-full object-contain" /> 
                                    </div> 
